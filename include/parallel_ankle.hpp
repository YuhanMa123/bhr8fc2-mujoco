...
class ParallelAnkle {
 public:
  ...
  struct AnkleParameters {
    T l_bar1 = 0.1;  // the length of the first bar
    T l_rod1 = 0.2;  // the length of the first rod
    Eigen::Matrix<T, 3, 1> r_a1 =
        Eigen::Matrix<T, 3, 1>::Zero();  // the initial position of point a1
                                         // ,but i have no idea of the named
                                         // format for the point
